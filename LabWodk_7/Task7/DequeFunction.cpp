...
DWORD WINAPI inputStartElements(LPVOID ptr)
{
	...
	while (true)
	{
		...
		// Запись элемента в дэк
		AddEnd(deque, deque->buffer);
		deque->length++;
		
		thrdSub = GetCurrentThread();
		std::cout << "Поток: " << GetThreadId(thrdSub) << std::endl;
		std::cout << "Элемент добавлен в деку: " << deque->buffer << std::endl;
		std::cout << "Дека:";
		OutputAll(deque);
		std::cout << "Запись деки в файл" << std::endl;
		WriteToFile(deque);
		std::cout << "Дека успешно записана в файл" << std::endl;
		std::cout << "Чтение деки из файла" << std::endl;
		ReadFromFile(deque);
		std::cout << "Дека успешно считана из файла. Результат: ";
		OutputAll(deque);
		std::cout << "***********************************************" << std::endl;
...
{
	cout << "Первый элемент дека: " << deque->deque.front() << endl; // Выводим первый элемент
}
